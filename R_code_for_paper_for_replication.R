...
#hist(p.POP[DATA.val$y == 0], col = 'blue', main = '', xlab = 'predicted probability', axes = F, ylab = '')
#hist(p.POP[DATA.val$y == 1], add = T, col = 'red')
#axis(1)
...
round(quantile(colMeans(p.EN[,,1], na.rm = T), probs = c(0.025, 0.975), na.rm = T), 2)
round(quantile(colMeans(p.EN[,,2], na.rm = T), probs = c(0.025, 0.975), na.rm = T), 2)
round(quantile(colMeans(p.EN[,,3], na.rm = T), probs = c(0.025, 0.975), na.rm = T), 2)
round(quantile(colMeans(p.EN[,,4], na.rm = T), probs = c(0.025, 0.975), na.rm = T), 2)
round(quantile(colMeans(p.EN[,,5], na.rm = T), probs = c(0.025, 0.975), na.rm = T), 2)
round(quantile(colMeans(p.EN[,,6], na.rm = T), probs = c(0.025, 0.975), na.rm = T), 2)

round(quantile(apply(p.EN[,, 1], 2, sd), probs = c(0.025, 0.975), na.rm = T), 2)
round(quantile(apply(p.EN[,, 2], 2, sd), probs = c(0.025, 0.975), na.rm = T), 2)
round(quantile(apply(p.EN[,, 3], 2, sd), probs = c(0.025, 0.975), na.rm = T), 2)
round(quantile(apply(p.EN[,, 4], 2, sd), probs = c(0.025, 0.975), na.rm = T), 2)
round(quantile(apply(p.EN[,, 5], 2, sd), probs = c(0.025, 0.975), na.rm = T), 2)
round(quantile(apply(p.EN[,, 6], 2, sd), probs = c(0.025, 0.975), na.rm = T), 2)

mean.pred.1 <- apply(p.EN[,, 1], 2, mean)
mean.pred.2 <- apply(p.EN[,, 2], 2, mean)
mean.pred.3 <- apply(p.EN[,, 3], 2, mean)
mean.pred.4 <- apply(p.EN[,, 4], 2, mean)
mean.pred.5 <- apply(p.EN[,, 5], 2, mean)
mean.pred.6 <- apply(p.EN[,, 6], 2, mean)

png(height=1200, width=1200, pointsize=15, file="mygraph.png")
par(mfrow = c(1, 1))
plot(density(mean.pred.1, na.rm = T), lty = 1, main = '', axes = F, ylim = c(0, 75), ann = F)
lines(density(mean.pred.2, na.rm = T), lty = 2)
lines(density(mean.pred.3, na.rm = T), lty = 3)
lines(density(mean.pred.4, na.rm = T), lty = 4)
lines(density(mean.pred.5, na.rm = T), lty = 5)
lines(density(mean.pred.6, na.rm = T), lty = 6)
axis(side = 1, at = seq(0, 1, 0.1)) 
legend("topright", legend = c("N=50", "N=100", "N=385", "N=500", "N=1000", "N=5000"), lty = 1:6, box.lty = 0, adj=c(0, 0.1))
title(xlab='Mean estimated risk')
box()
dev.off()
...
for(j in 2:5){
  X <- p.EN[,,j]
  x <- density(X[,1])
  plot(x, ylim = c(0, 4), lwd = 0.05, xlab = '', ylab = '', main = paste('N = ', N.DEV[j]), sep='')
  for(i in 2:N.SIM){
    x <- density(X[,i])
    lines(x, lwd = 0.05)
  }
...
#### subgroup (risks in those with X1 < -1)
p.EN.subgroup <- matrix(ncol = N.SIM, nrow = nrow(DATA.val))
pb <- progress_bar$new(
  format = " simulation done [:bar] :percent eta: :eta",
  total = N.SIM, clear = FALSE, width = 60)

for(i in 1:N.SIM){
  pb$tick()
  DATA  <- generate_data(NN = N.DEV[2], n.true.predictors = n.true.predictors, n.noise.predictors = n.noise.predictors)
  fit.EN <- EN_model(data.d = DATA, data.v = DATA.val)
  if(length(class(fit.EN$fit))>1){
    p.EN.subgroup[,i] <- predict(fit.EN$fit, newdata = DATA.val, type = 'fitted')
  } else { # models that don't converge
    p.EN.subgroup[,i] <- rep(NA, nrow(DATA.val))
  }
}

OUT <- vector(mode = 'numeric', length = N.SIM)
for(i in 1:N.SIM){
  XX <- p.EN.subgroup[,i]
  OUT[i] <- mean(XX[DATA.val$X1 < -1])
}
plot(density(OUT))
...
#OUT %>% filter(N.DEV=='100') %>% ggplot(aes(y = value, x = prob)) +
...
OUT %>% group_by(N.DEV, prob) %>% summarise(mean = mean(value, na.rm = T)) %>% pivot_wider(names_from = prob, values_from = mean) %>% print(n = 100)
OUT %>% group_by(N.DEV, prob) %>% summarise(var = var(value, na.rm = T)) %>% pivot_wider(names_from = prob, values_from = var) %>% print(n = 100)
OUT %>% group_by(N.DEV, prob) %>% summarise(IQR = IQR(value, na.rm = T)) %>% pivot_wider(names_from = prob, values_from = IQR) %>% print(n = 100) 
OUT %>% filter(N.DEV==50) %>% group_by(N.DEV, prob) %>% summarise(Q1 = quantile(value, prob = 0.25, na.rm = T), Q3= quantile(value, prob = 0.75, na.rm = T)) %>% pivot_wider(names_from = prob, values_from = c(Q1, Q3)) %>% print(n = 100) 
